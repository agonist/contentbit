...
- **`renderMarkdown`** — `(md: string) => string | Promise<string>` for prose
  and block bodies. Pass your site's own Markdown pipeline to match the rest of
  your output. The minimal fallback is exported as `defaultRenderMarkdown` from
  `@contentbit/astro`.
