...
- **`renderMarkdown`** — `(md: string) => string` for prose and block bodies.
  Ships with a marked-based default (GFM, raw HTML escaped), so it works
  untouched; swap in your own to match the rest of your site. The default is
  also exported as `defaultRenderMarkdown` from `@contentbit/astro`.
