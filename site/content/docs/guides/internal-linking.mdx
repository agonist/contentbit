...
The JSON artifact is intentionally small and stable, so an LLM agent or pipeline can
...
- `CB_LINK_LOCALE_MISSING` for a target that exists in another locale but not the
  current locale.
- `CB_LINK_CROSS_LOCALE` for an explicit object target that intentionally points
  across locales.
...
3. For new linked pages, include `keywords.primary` and `keywords.secondary`
   with search-intent phrases that help future LLM agents choose related pages.
4. Run `contentbit links <content glob> --fix` after edits when aliases are
   present.
5. Run `contentbit validate <content glob> --registry <path>` until clean.

For projects without generated LLM-agent files, add the same loop to your own skill or
