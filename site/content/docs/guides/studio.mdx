...
The generated `contentbit.config.ts` supplies the content glob and registry, so
...
Studio is a separate development package so the base CLI stays lightweight.
...
Studio renders previews through `@contentbit/react`. Blocks are validated from
your registry, and Studio automatically looks for project React components next
to the registry (`blocks/components.tsx` from `contentbit init`,
...
This includes Astro-only renderers: `.astro` files render in the Astro app, while
Studio needs a React preview renderer or uses the labelled fallback.
