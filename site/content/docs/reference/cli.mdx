...
Put shared project settings in `contentbit.config.ts`:
...
Commands discover the nearest config by walking upward. Explicit options and
globs override it for one invocation. Quote explicit globs so your shell does
not expand them before contentbit receives them:

```bash
contentbit validate "content/**/*.md" --registry ./blocks/registry.ts
```

Without a config, `validate`, `doctor`, `studio`, and `links` retain backward
compatibility by reading the nearest matching package script.
...
| `adopt` | Inspect an existing Markdown library and propose read-only setup. |
...
Inspect an established Markdown library and print reviewable configuration
proposals. This command is always read-only: it never creates, changes, or
renames files.

```bash
contentbit adopt [globs...] [--dry-run] [--json]
```

Without a glob, it scans `content/**/*.{md,mdx}`. The report counts common
frontmatter fields, infers a locale-aware link strategy when appropriate,
shows stable-key locale coverage, and includes a proposed `contentbit.config.ts`.
Projects with observed page `type` values also receive a baseline SEO contract
inferred from consistently present frontmatter and sections. Review that
baseline before enforcing it. See [Adopt an existing content library](/docs/guides/adoption).

| Option | Meaning |
| --- | --- |
| `--registry <module>` | Load custom block definitions while scanning. |
| `--no-generic-blocks` | Do not load the generic block pack first. |
| `--dry-run` | Explicitly confirm the report-only workflow; adoption is always read-only. |
...
```bash
contentbit doctor [globs...] [--watch] [--registry <module.ts>] [--json]
```
...
| `--watch` | Rerun when a scanned content directory changes. |
