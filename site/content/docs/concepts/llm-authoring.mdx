...
There's a third leg for existing content: [`contentbit stats`](/docs/guides/stats)
reports outlines, block usage, and validation summaries as JSON, so an LLM agent
can rank what needs attention across the whole content directory.
...
clean, audit with stats. See [LLM-agent integration](/docs/guides/agents).
